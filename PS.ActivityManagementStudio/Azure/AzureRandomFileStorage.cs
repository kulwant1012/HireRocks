...
namespace PS.ActivityManagementStudio.Azure
{
    public class AzureRandomFileStorage
    {
        ...
        {
            var blobName = _basePath + Guid.NewGuid().ToString("N");
        ...
        public async Task ReadAsync(string uri, Stream data)
        {
            if (!uri.StartsWith(_containerPath))
                throw new ArgumentException(String.Format("Uri does not match container url. Container: {0} Blob: {1}", _containerPath, uri), "uri");
            var blobname = uri.Substring(_containerPath.Length + 1);
            var blob = Container.GetBlockBlobReference(blobname);
